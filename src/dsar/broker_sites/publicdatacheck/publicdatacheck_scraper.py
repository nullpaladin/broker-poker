# publicdatacheck.com — help-center/privacy-requests. Shares the identical
# white-label form template as kidslivesafe.com elsewhere in this repo
# (same field ids down to the obfuscated last-name field), just under a
# different brand. "Request Type" select (Do Not Sell My Info / Delete My
# Info / Request a Copy) — one submission per type. First/Last/City/State/
# ZIP/Age are always required; the Last Name field's real `name`/`id` is a
# random hex string ("ac5ccdde8ce2a780d" at the time this was written, may
# change) — an anti-bot obfuscation technique, not a stable identifier, so
# it's targeted positionally (the 3rd plain `input[type=text]` on the page
# — order is customerId, firstName, Last Name, city, zip) rather than by
# name/id. Email and an optional Customer ID field only appear (JS-revealed)
# for Delete/Request a Copy — those two rights also open an "Are you a
# Customer?" modal after a successful submit (not reached here since the
# Turnstile blocks it) that, per the page's own copy, tells non-customers
# they have nothing on file and to use Do Not Sell instead — a genuine
# customer-only limitation for those two rights, not a scraper bug. Age is a
# numeric select (18-105) — DATE_OF_BIRTH is converted to an age and clamped
# into that range. A Cloudflare Turnstile checkbox gates the "Continue"
# button — **CAPTCHA solution required**; the form is filled completely
# regardless and left at the CAPTCHA per this repo's standard policy.
# Exercises Do Not Sell and Request a Copy unconditionally; Delete gated on
# REMOVE_INFORMATION.
import asyncio
